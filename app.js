  /* =========================================================
     영구 저장소 요청 — 이걸 안 하면 브라우저가 디스크 용량이
     부족할 때 "best-effort" 저장소로 취급해서 IndexedDB/Cache
     Storage(=다운로드한 모델 파일이 저장되는 곳)를 브라우저가
     알아서 지워버릴 수 있음. 이게 "매번 모델을 다시 받는" 증상의
     가장 흔한 원인이라 앱 시작 시 최대한 빨리 요청함.
  ========================================================= */
  (async () => {
    try{
      if (navigator.storage?.persist){
        ...
        if (!already){
          ...
          console.log(granted
            ? '[저장소] 영구 저장소 승인됨 — 모델 캐시가 임의로 삭제되지 않아요'
            : '[저장소] 영구 저장소 요청이 거부됨 — 브라우저 정책에 따라 캐시가 지워질 수 있어요');
        }
      }
    }catch(e){
      console.warn('[저장소] persist 요청 실패', e);
    }
  })();
  ...
  const screenShareBar = document.getElementById('screen-share-bar');
  const screenSharePreview = document.getElementById('screen-share-preview');
  const screenShareStopBtn = document.getElementById('screen-share-stop');
  ...
  const webgpuModelConvoSelect = document.getElementById('webgpu-model-convo');
  const webgpuModelResearchSelect = document.getElementById('webgpu-model-research');
  const webgpuModelVisionSelect = document.getElementById('webgpu-model-vision');
  const webgpuRefreshBtn = document.getElementById('webgpu-refresh');
  const webgpuTestBtn = document.getElementById('webgpu-test-btn');
  const webgpuStatusHint = document.getElementById('webgpu-status-hint');
  ...
  function buildSystemPrompt(){
    ...
    let prompt =
      ...
      "질문에는 웹 검색 결과가 이미 함께 주어져. 너는 그 결과 안에서만 답하면 되고, " +
      "직접 검색을 실행하거나 다시 검색할 수는 없어. '다시 찾아볼게요', '조금만 기다려주세요', " +
      "'확인해보겠습니다' 같이 나중에 뭔가 더 하겠다는 말은 하지 마. " +
      "주어진 검색 결과로 바로 답하고, 결과에 없으면 그 자리에서 바로 모른다고 솔직히 말해줘.";
  ...
  function getConvoModelId(){
    return localStorage.getItem('sodam_webgpu_model_convo') || '';
  }
  function getResearchModelId(){
    return localStorage.getItem('sodam_webgpu_model_research') || '';
  }
  function getVisionModelId(){
    return localStorage.getItem('sodam_webgpu_model_vision') || '';
  }

  /* =========================================================
     WebGPU 인브라우저 AI (WebLLM) — 서버·Ollama 없이 이 브라우저
     안에서 직접 모델을 돌림. GitHub Pages 같은 정적 사이트로 열어도
     그대로 동작함(별도 백엔드 불필요). 최신 Chrome/Edge(WebGPU 지원)
     필요.
  ========================================================= */
  let webllmModPromise = null;
  function loadWebLLM(){
    if (!webllmModPromise){
      webllmModPromise = import('https://esm.run/@mlc-ai/web-llm');
    }
    return webllmModPromise;
  }
  ...
  let mlcEngine = null;
  let mlcEngineModelId = null;
  let mlcEngineLoading = null; // { modelId, promise }

  function makeProgressCaption(prefix){
    return (p) => {
      const pct = Math.round((p?.progress || 0) * 100);
      setCaption(`${prefix} 준비 중… ${pct}% (처음 한 번만 다운로드하고 이후엔 브라우저에 저장돼요)`, true);
    };
  }

  async function ensureEngineForModel(modelId, onProgress){
    if (!modelId) throw new Error('모델이 선택되지 않았어요. 설정 > AI 모델에서 모델을 선택해주세요.');
    if (!isWebGPUSupported()){
      throw new Error('이 브라우저는 WebGPU를 지원하지 않아요. 최신 Chrome 또는 Edge로 열어주세요.');
    }
    if (mlcEngine && mlcEngineModelId === modelId) return mlcEngine;
    if (mlcEngineLoading && mlcEngineLoading.modelId === modelId) return mlcEngineLoading.promise;

    const webllm = await loadWebLLM();
    const promise = (async () => {
      if (mlcEngine){
        try{ await mlcEngine.unload(); }catch(e){ /* 무시하고 계속 진행 */ }
        mlcEngine = null;
        mlcEngineModelId = null;
      }
      const engine = await webllm.CreateMLCEngine(modelId, { initProgressCallback: onProgress });
      mlcEngine = engine;
      mlcEngineModelId = modelId;
      return engine;
    })();
    mlcEngineLoading = { modelId, promise };
    try{
      return await promise;
    }finally{
      mlcEngineLoading = null;
    }
  }

  // 모델 이름으로 "생각(추론)" 모델인지 대략 판별 — 이런 모델은 최종 답을
  // 내놓기 전에 <think>...</think> 안에서 긴 추론을 먼저 쏟아내기 때문에
  // 토큰 예산이 부족하면 추론 도중에 잘려서 </think>가 끝내 안 나오고,
  // 그 잘린 추론 전체가 그대로 답변으로 노출돼버림. 이런 모델에는 넉넉한
  // 토큰 예산을 줘서 추론을 끝까지 마치고 실제 답까지 갈 수 있게 함.
  function isThinkingModelId(modelId){
    return /think|reason|r1|qwq|deepseek-r1/i.test(String(modelId || ''));
  }

  // <think>...</think> 제거 — 정상적으로 닫힌 태그뿐 아니라, 토큰이 모자라
  // 답변 도중 잘려서 </think>가 아예 없는 경우(추론이 안 끝난 채로 끊긴 경우)도
  // 처리함: 이때는 <think> 이후 전부가 사실상 "생각 중"이었던 내용이라
  // 사용자에게 보여줄 완성된 답이 아직 없는 것이므로 통째로 버림.
  function stripThinking(text){
    if (!text) return text;
    let out = text.replace(/<think>[\s\S]*?<\/think>/gi, '');
    const openIdx = out.search(/<think>/i);
    if (openIdx !== -1) out = out.slice(0, openIdx); // 닫히지 않은 <think> — 그 이후는 전부 버림
    return out.trim();
  }

  async function webllmChat(modelId, systemContent, hist, prompt){
    const engine = await ensureEngineForModel(modelId, makeProgressCaption('대화 모델'));
    setCaption('생성하는 중…', true);
    // 생각 모델은 추론 + 실제 답변을 다 쓸 수 있게 토큰 예산을 훨씬 넉넉하게 줌
    const maxTokens = isThinkingModelId(modelId) ? 2000 : 800;
    ...
    if (text) text = stripThinking(text);
    if (!text){
      // 추론만 하다 토큰이 다 떨어져서 실제 답이 하나도 안 나온 경우 —
      // 조용히 빈 답으로 넘기지 않고 명확한 이유를 남겨서, 호출부가 이걸
      // 실패로 보고 다음 단계(온디바이스 등)로 자연스럽게 폴백하게 함.
      throw new Error(isThinkingModelId(modelId)
        ? 'WebGPU 모델: 생각(추론)만 하다가 토큰이 부족해 답을 못 냈어요'
        : 'WebGPU 모델: 빈 응답이 왔어요');
    }
    return text;
  }

  async function webllmVisionDescribe(modelId, prompt, dataUrl){
    if (!dataUrl) throw new Error('이미지 데이터를 읽지 못했어요');
    const engine = await ensureEngineForModel(modelId, makeProgressCaption('비전 모델'));
    ...
    if (!text) throw new Error('WebGPU 비전 모델: 빈 응답이 왔어요 (비전 지원 모델이 맞는지 확인해주세요)');
    return text;
  }

  // 설정 화면의 모델 선택 목록을 WebLLM에 등록된 모델 목록으로 채움
  async function populateWebGPUModelSelects(force){
    if (!webgpuModelConvoSelect) return;
    if (webgpuModelConvoSelect.dataset.loaded && !force) return;
    const loadingOpt = '<option value="">모델 목록 불러오는 중…</option>';
    webgpuModelConvoSelect.innerHTML = loadingOpt;
    webgpuModelResearchSelect.innerHTML = loadingOpt;
    webgpuModelVisionSelect.innerHTML = loadingOpt;
    try{
      const webllm = await loadWebLLM();
      const all = webllm.prebuiltAppConfig?.model_list || [];
      const isVision = (m) => /vision/i.test(m.model_id);
      const byVram = (a, b) => (a.vram_required_MB || 0) - (b.vram_required_MB || 0);
      const textModels = all.filter(m => !isVision(m)).sort(byVram);
      const visionModels = all.filter(isVision).sort(byVram);
      const optionHtml = (m) => {
        const gb = m.vram_required_MB ? ` · 약 ${(m.vram_required_MB / 1024).toFixed(1)}GB` : '';
        const thinkTag = isThinkingModelId(m.model_id) ? ' · 🧠생각모델(느림)' : '';
        return `<option value="${m.model_id}">${m.model_id}${gb}${thinkTag}</option>`;
      };
      const textOptions = '<option value="">사용 안 함</option>' + textModels.map(optionHtml).join('');
      const visionOptions = '<option value="">사용 안 함</option>' + visionModels.map(optionHtml).join('');
      webgpuModelConvoSelect.innerHTML = textOptions;
      webgpuModelResearchSelect.innerHTML = textOptions;
      webgpuModelVisionSelect.innerHTML = visionOptions;
      webgpuModelConvoSelect.dataset.loaded = '1';
    }catch(err){
      console.error('[WebLLM 모델 목록 로드 실패]', err);
      const errOpt = '<option value="">목록을 불러오지 못했어요 (네트워크 확인)</option>';
      webgpuModelConvoSelect.innerHTML = errOpt;
      webgpuModelResearchSelect.innerHTML = errOpt;
      webgpuModelVisionSelect.innerHTML = errOpt;
    }
  }

  async function testWebGPUSupport(){
    webgpuStatusHint.textContent = 'WebGPU 확인 중…';
    if (!isWebGPUSupported()){
      webgpuStatusHint.textContent = '이 브라우저는 WebGPU를 지원하지 않아요. 최신 Chrome 또는 Edge(버전 113 이상)에서 열어주세요.';
      return;
    }
    try{
      const adapter = await navigator.gpu.requestAdapter();
      if (!adapter){
        webgpuStatusHint.textContent = 'WebGPU API는 있지만 사용 가능한 그래픽 어댑터를 찾지 못했어요.';
        return;
      }
      const info = adapter.info || {};
      const who = [info.vendor, info.architecture].filter(Boolean).join(' · ');
      webgpuStatusHint.textContent = `WebGPU 사용 가능해요${who ? ` (${who})` : ''}. 모델을 고르고 대화를 시작하면 처음 한 번만 자동으로 다운로드돼요.`;
    }catch(err){
      webgpuStatusHint.textContent = `확인 실패: ${err.message || err}`;
    }
  }
  webgpuTestBtn?.addEventListener('click', testWebGPUSupport);
  webgpuRefreshBtn?.addEventListener('click', () => {
    populateWebGPUModelSelects(true).then(() => {
      webgpuModelConvoSelect.value = getConvoModelId();
      webgpuModelResearchSelect.value = getResearchModelId();
      webgpuModelVisionSelect.value = getVisionModelId();
    });
  });
  ...
  /* =========================================================
     비전(사진 설명) 호출 — WebGPU(WebLLM)로 브라우저 안에서 직접 처리
  ========================================================= */
  async function callVision(prompt, dataUrl){
    const model = getVisionModelId();
    if (!model){
      setPipelineStep('vision','skip');
      throw new Error('비전용 모델이 설정되지 않았어요. 설정 > AI 모델에서 비전용 모델을 선택해주세요.');
    }
    setPipelineStep('vision','active');
    setCaption('사진을 살펴보는 중…', true);
    const cmd = `WebGPU(브라우저) 추론\nmodel: ${model} (vision)`;
    try{
      const text = await webllmVisionDescribe(model, prompt, dataUrl);
      ...
      return { text, tier:'vision', label:'브라우저(WebGPU) · 사진 설명' };
    }catch(err){
  ...
  /* =========================================================
     실시간 화면 인식 (화면 공유)
     - getDisplayMedia로 화면 스트림을 받아서 숨겨진 <video>에 연결해두고,
       사용자가 뭔가 물어볼 때마다 그 순간의 화면을 캡처(canvas)해서
       비전(사진 설명) 모델에 "지금 화면"으로 함께 보냄.
     - 진짜 실시간 영상 이해는 무료 로컬 모델로는 너무 무겁고 느려서,
       "물어볼 때마다 최신 화면 한 장을 같이 보고 답한다" 방식으로 구현함.
       (사진 첨부에 쓰는 것과 같은 브라우저 내 비전 모델을 재사용 — 별도 설정 필요 없음)
     - Electron 창으로 뜬 경우엔 main.js에서 setDisplayMediaRequestHandler를
       등록해둬야 화면 선택이 되고, 진짜 Chrome 앱 모드로 뜬 경우엔
       브라우저가 알아서 화면 선택 창을 띄워줌.
  ========================================================= */
  ...
  function updateScreenShareUI(active){
    screenShareBar.style.display = active ? 'flex' : 'none';
    screenShareBtn.classList.toggle('active', active);
    screenShareBtn.setAttribute('aria-pressed', active ? 'true' : 'false');
    screenShareBtn.title = active ? '화면 공유 중지' : '화면 공유해서 대화하기';
  }

  function drawScreenPreview(){
    if (!screenShareActive || !screenVideoEl || !screenVideoEl.videoWidth){
      screenPreviewRAF = requestAnimationFrame(drawScreenPreview);
      return;
    }
    const ctx = screenSharePreview.getContext('2d');
    const w = screenSharePreview.width, h = screenSharePreview.height;
  ...
  async function startScreenShare(){
    ...
    if (!getVisionModelId()){
      alert('먼저 설정 > AI 모델에서 비전용 모델을 선택해주세요. 화면 공유 대화에도 그 모델을 써요.');
      return;
    }
    if (!navigator.mediaDevices?.getDisplayMedia){
      alert('이 화면에서는 화면 공유가 지원되지 않아요.');
      return;
    }
    try{
      ...
      console.warn('[화면 공유] 취소되었거나 실패:', err.message || err);
      return;
    }
    ...
    try{ await screenVideoEl.play(); }catch{ /* 자동재생 정책에 걸려도 캡처엔 지장 없음 */ }
  ...
  function stopScreenShare(){
    ...
    if (screenStream){
      try{ screenStream.getTracks().forEach(t => t.stop()); }catch{ /* 무시 */ }
    }
    screenStream = null;
    screenVideoEl = null;
  ...
  // 화면 캡처 1장을 base64 dataURL(jpeg)로 반환 — 크기를 줄여서
  // 로컬 모델 응답 속도를 빠르게 하고, 대화 기록 용량도 아낌
  function captureScreenFrame(){
    if (!screenVideoEl || !screenVideoEl.videoWidth) return null;
    const maxWidth = 1024;
    const scale = Math.min(1, maxWidth / screenVideoEl.videoWidth);
    const w = Math.max(1, Math.round(screenVideoEl.videoWidth * scale));
    const h = Math.max(1, Math.round(screenVideoEl.videoHeight * scale));
    const canvas = document.createElement('canvas');
    canvas.width = w; canvas.height = h;
    canvas.getContext('2d').drawImage(screenVideoEl, 0, 0, w, h);
  ...
  /* =========================================================
     DuckDuckGo 웹 검색
     - Electron(데스크톱 앱)으로 실행 중이면 main.js의 로컬 서버가
       대신 요청해주는 /api/search를 먼저 써봄 (CORS 회피, 더 빠르고 안정적)
     - GitHub Pages 같은 순수 정적 사이트로 열려서 /api/search가 없으면,
       브라우저에서 공개 CORS 프록시를 거쳐 DuckDuckGo 결과 페이지를
       직접 받아와 파싱함 (백엔드 서버 없이도 동작)
  ========================================================= */
  async function searchWeb(query, count){
    ...
    try{
      ...
      if (res.ok){
        ...
        if (!data.error) return data.results || [];
      }
    }catch(err){
      // 로컬 서버가 없는 정적 사이트 환경 — 아래 브라우저 직접 검색으로 폴백
    }
    return searchWebViaBrowser(query, n);
  }
  ...
  function decodeDdgLinkInBrowser(rawHref){
    try{
  ...
  function parseDuckDuckGoHtml(html, max){
    ...
    while ((m = re.exec(html)) && results.length < max){
      results.push({
        title: stripSearchHtml(m[2]),
        snippet: stripSearchHtml(m[3]),
        link: decodeDdgLinkInBrowser(m[1])
      });
    }
    return results;
  }
  // 공개 CORS 프록시는 자주 막히거나(레이트리밋) DuckDuckGo가 프록시 IP를
  // 캡차/차단 페이지로 돌려보내는 경우가 있음. 이럴 때도 fetch 자체는 200
  // OK로 성공하기 때문에, 그동안은 "검색 성공 + 결과 0건"으로 조용히
  // 넘어가서 실제로는 검색이 안 됐는데도 실패를 알아챌 수 없었음.
  // → 프록시를 여러 개 순서대로 시도하고, 결과가 0건이면 차단/마크업
  //   변경으로 간주해 다음 프록시로 재시도. 전부 실패하면 에러를 던져서
  //   호출부(callResearch)가 "검색 실패"로 명확히 로그를 남기게 함.
  const DDG_CORS_PROXIES = [
    (url) => 'https://api.allorigins.win/raw?url=' + encodeURIComponent(url),
    (url) => 'https://corsproxy.io/?url=' + encodeURIComponent(url),
    (url) => 'https://api.codetabs.com/v1/proxy?quest=' + encodeURIComponent(url)
  ];

  async function fetchDdgHtmlOnce(query, buildProxyUrl){
    ...
    if (!res.ok) throw new Error(`검색 프록시 HTTP ${res.status}`);
    const html = await res.text();
    if (!html || html.length < 200) throw new Error('검색 프록시가 빈 응답을 돌려줬어요');
    return html;
  }

  async function searchWebViaBrowser(query, n){
    let lastErr = null;
    for (const buildProxyUrl of DDG_CORS_PROXIES){
      try{
        const html = await fetchDdgHtmlOnce(query, buildProxyUrl);
        const results = parseDuckDuckGoHtml(html, n);
        if (results.length) return results;
        // fetch는 성공했지만 파싱 결과가 0건 — DuckDuckGo가 이 프록시를
        // 차단(캡차/안내 페이지)했거나 마크업이 바뀌었을 가능성이 큼.
        // "진짜 결과 없음"과 구분이 안 되니 다음 프록시로 넘어가봄.
        console.warn('[검색] 결과 0건 — 프록시 차단/마크업 변경 의심, 다음 방법으로 재시도');
        lastErr = new Error('검색 결과 파싱 0건 (DuckDuckGo 차단 또는 페이지 구조 변경 가능성)');
      }catch(err){
        console.warn('[검색] 프록시 실패, 다음 방법으로 재시도', err.message || err);
        lastErr = err;
      }
    }
    throw lastErr || new Error('검색에 실패했어요');
  }
...
- 풍속: ${c.wind_speed_10m}m/s${maxPop !== null ? `\n- 앞으로 6시간 내 최고 강수확률: ${maxPop}%` : ''}`,
  ...
  /* =========================================================
     공급자 2: 자료 검색용 모델(WebGPU/WebLLM) + DuckDuckGo
  ========================================================= */
  async function callResearch(prompt, hist){
    const model = getResearchModelId();
    ...
    const systemContent = buildSystemPrompt() + (weatherQuery && !resultsText.startsWith('(검색 실패')
      ...
      : `\n\n아래는 방금 웹에서 검색한 결과야. 이게 네가 볼 수 있는 검색 결과의 전부이고, 너는 지금 이 한 번의 대답으로 끝이야 — 추가로 검색을 실행할 수 없어. ` +
        `이 내용을 참고해서 지금 바로 답하고, 검색 결과에 없는 내용이면 "다시 찾아볼게요", "기다려주세요" 같은 말 대신 그냥 모른다고 솔직히 말해줘.\n\n[검색 결과]\n${resultsText}`);
    const chatCmd = `WebGPU(브라우저) 추론\nmodel: ${model}`;
    try{
      const text = await webllmChat(model, systemContent, hist, prompt);
      setPipelineStep('research','ok');
      setPipelineStep('ondevice','skip');
      logExec(true, '자료 검색 모델 호출', chatCmd, text.slice(0, 300));
      return { text, tier:'research', label: weatherQuery ? '브라우저(WebGPU) · 실시간 날씨' : '브라우저(WebGPU) · 자료 검색' };
    }catch(err){
  ...
  function isDeepseekModelSelected(){
    const convo = getConvoModelId().toLowerCase();
    const research = getResearchModelId().toLowerCase();
  ...
  attachInput.addEventListener('change', async () => {
    ...
    if (isImageFile(file)){
      ...
      if (!visionModel){
        showAttachChip(file.name, '설정 > AI 모델에서 비전용 모델을 먼저 선택해주세요');
        pendingAttachment = null;
        return;
      }
  ...
  async function handleUserInput(rawText){
    ...
    // 첨부 파일이 없는데 화면 공유 중이면, 지금 이 순간의 화면을 캡처해서
    // 사진 첨부와 똑같은 방식(비전 모델)으로 처리함 — 매번 다시 첨부할 필요 없음
    if (!attachment && screenShareActive){
      const frame = captureScreenFrame();
      if (frame){
        attachment = { isImage: true, isScreen: true, name: '실시간 화면', dataUrl: frame };
      }
    }

    if (!userText && !attachment) return;
    if (!userText && attachment){
      userText = attachment.isScreen ? '지금 화면에 뭐가 보이는지 설명해줘.'
        : (attachment.isImage ? '이 사진에 무엇이 보이는지 설명해줘.' : '이 파일의 내용을 설명하고 핵심을 요약해줘.');
    }
    const myId = ++currentRequestId;

    // 이미지 첨부(또는 화면 공유 캡처)는 일반 텍스트 파이프라인을 타지 않고 비전 모델로 바로 처리함
    if (attachment?.isImage){
      ensureConversation(userText);
      // 화면 캡처는 매번 채팅창에 큰 스크린샷을 남기지 않고, 일반 사진 첨부만 썸네일로 보여줌
      appendMessage('나', userText, 'user', null, null, attachment.isScreen ? null : attachment.dataUrl);
      setCaption(userText, true);
      hideAttachChip();

      history.push({ role:'user', content: `[${attachment.isScreen ? '실시간 화면 캡처' : '사진 첨부: ' + attachment.name}] ${userText}` });
      ...
      try{
        ...
        console.error(err);
        hidePipelineSoon();
        const failMsg = attachment.isScreen
          ? '죄송해요, 화면을 설명하지 못했어요. 설정에서 비전 모델을 확인해주세요.'
          : '죄송해요, 사진을 설명하지 못했어요. 설정에서 비전 모델을 확인해주세요.';
        appendMessage('AI', failMsg, 'ai-error', null, currentExecLog.slice());
        setCaption(attachment.isScreen ? '화면 설명에 실패했어요' : '사진 설명에 실패했어요', false);
        setOrbState(handsFreeOn ? 'listening' : 'idle');
      }
      return;
    }
  ...
  function openSettings(){
    webgpuStatusHint.textContent = '아직 확인하지 않았어요. "테스트"를 눌러 WebGPU 사용 가능 여부를 확인해보세요.';
    populateWebGPUModelSelects().then(() => {
      webgpuModelConvoSelect.value = getConvoModelId();
      webgpuModelResearchSelect.value = getResearchModelId();
      webgpuModelVisionSelect.value = getVisionModelId();
    });
  ...
  document.getElementById('save-settings').addEventListener('click', () => {
    const mc = webgpuModelConvoSelect.value;
    if (mc) localStorage.setItem('sodam_webgpu_model_convo', mc);
    else localStorage.removeItem('sodam_webgpu_model_convo');
    const mr = webgpuModelResearchSelect.value;
    if (mr) localStorage.setItem('sodam_webgpu_model_research', mr);
    else localStorage.removeItem('sodam_webgpu_model_research');
    const mv = webgpuModelVisionSelect.value;
    if (mv) localStorage.setItem('sodam_webgpu_model_vision', mv);
    else localStorage.removeItem('sodam_webgpu_model_vision');
  ...
  resetSettingsBtn.addEventListener('click', () => {
    localStorage.removeItem('sodam_webgpu_model_convo');
    localStorage.removeItem('sodam_webgpu_model_research');
    localStorage.removeItem('sodam_webgpu_model_vision');
    webgpuModelConvoSelect.value = '';
    webgpuModelResearchSelect.value = '';
    webgpuModelVisionSelect.value = '';
    webgpuStatusHint.textContent = '아직 확인하지 않았어요. "테스트"를 눌러 WebGPU 사용 가능 여부를 확인해보세요.';
  });
