...
(function(){
  ...
  /* ---------------------------------------------------------
     Ambient effects — drifting frost specks in the sky layer
  --------------------------------------------------------- */
  function spawnFrostSpecks(){
    const sky = document.getElementById("contentSky");
    if(!sky) return;
    const count = 14;
    for(let i=0;i<count;i++){
      const s = document.createElement("span");
      s.className = "frost-speck";
      s.style.left = (Math.random()*100).toFixed(1)+"%";
      s.style.top = (-Math.random()*100).toFixed(0)+"px";
      s.style.animationDuration = (14+Math.random()*16).toFixed(1)+"s";
      s.style.animationDelay = (-Math.random()*20).toFixed(1)+"s";
      s.style.opacity = (0.2+Math.random()*0.35).toFixed(2);
      if(Math.random() < 0.3) s.classList.add("gold");
      sky.appendChild(s);
    }
  }
