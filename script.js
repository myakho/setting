  ...
  async function callVision(prompt, dataUrl){
    const model = getCloudVisionModel();
    if (!model){
      setPipelineStep('vision','skip');
      throw new Error('비전용 모델이 설정되지 않았어요. 설정 > AI 연결에서 비전 모델을 등록해주세요.');
    }
    setPipelineStep('vision','active');
    setCaption('사진을 살펴보는 중…', true);
    const cmd = `POST ${getCloudBaseUrl()}/chat/completions\nmodel: ${model} (vision)`;
    try{
      const text = await cloudVisionDescribe(model, prompt, dataUrl);
      ...
      return { text, tier:'vision', label: '클라우드 · 사진 설명' };
    }catch(err){
...
- 풍속: ${c.wind_speed_10m}m/s${maxPop !== null ? `\n- 앞으로 6시간 내 최고 강수확률: ${maxPop}%` : ''}`,
  ...
  /* =========================================================
     공급자 2: 자료 검색용 모델 (클라우드 API) + DuckDuckGo
  ========================================================= */
  async function callResearch(prompt, hist){
    const model = getCloudResearchModel();
    ...
    const chatCmd = `POST ${getCloudBaseUrl()}/chat/completions\nmodel: ${model}`;
    try{
      const text = await cloudChat(model, systemContent, hist, prompt);
      setPipelineStep('research','ok');
      setPipelineStep('ondevice','skip');
      logExec(true, '자료 검색 모델 호출', chatCmd, text.slice(0, 300));
      return { text, tier:'research', label: weatherQuery ? '클라우드 · 실시간 날씨' : '클라우드 · 자료 검색' };
    }catch(err){
  ...
  /* =========================================================
     공급자 3 (최종 폴백): 온디바이스 (transformers.js, WebGPU, 키 불필요)
     - WebGPU가 없는 브라우저에서는 폴백 자체를 포기하고 바로 실패 처리함
       (CPU/WASM 경로는 속도·품질이 너무 떨어져서 최종 안전망으로 부적합하다고 판단)
  ========================================================= */
  ...
  async function callOnDevice(prompt, hist){
    ...
    if (!navigator.gpu){
      ...
      const msg = '이 브라우저는 WebGPU를 지원하지 않아 온디바이스 모델을 쓸 수 없어요 (최신 Chrome·Edge 등에서 이용해주세요)';
      logExec(false, '온디바이스 모델 준비', 'navigator.gpu 확인', msg);
      throw new Error(msg);
    }
    ...
    // WebGPU라 CPU/WASM보다 훨씬 빨라서, 자료 정리처럼 긴 답이 필요한 경우도
    // 어느 정도 감당할 수 있게 출력 상한을 180 → 400 토큰으로 올림
    const modelOutput = await onDevicePipeline(messages, { max_new_tokens: 400 });
    const generated = modelOutput?.[0]?.generated_text;
    const text = Array.isArray(generated) ? generated.at(-1)?.content : generated;
    const cmd = '온디바이스 추론 (onnx-community/Qwen2.5-0.5B-Instruct, q4, WebGPU)';
    if (!text){
      setPipelineStep('ondevice','fail');
  ...
  function isDeepseekModelSelected(){
    const convo = getCloudConvoModel().toLowerCase();
    const research = getCloudResearchModel().toLowerCase();
  ...
  attachInput.addEventListener('change', async () => {
    ...
    if (isImageFile(file)){
      const visionModel = getCloudVisionModel();
      if (!visionModel){
