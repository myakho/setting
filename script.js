  ...
  const providerTabOndevice = document.getElementById('provider-tab-ondevice');
  const aiProviderOllamaFields = document.getElementById('ai-provider-ollama-fields');
  const aiProviderOndeviceFields = document.getElementById('ai-provider-ondevice-fields');
  // 클라우드 API(Groq 등) 탭은 UI에서 없앴지만, 코드/필드는 그대로 남겨둠
  // (element가 없으면 null이 되고, 아래 코드들은 계속 안전하게 동작함)
  ...
  const ondeviceStatusHint = document.getElementById('ondevice-status-hint');
  const ondeviceWarmupBtn = document.getElementById('ondevice-warmup-btn');
  ...
  /* 로컬(Ollama) / 온디바이스(브라우저·WebGPU) 탭 전환
     클라우드 API 탭은 UI에서 없앴음 — aiProviderCloudFields는 코드만 남아있고 항상 숨김 상태 */
  function switchAiProviderTab(provider){
    const isOndevice = provider === 'ondevice';
    providerTabOllama.classList.toggle('active', !isOndevice);
    providerTabOndevice?.classList.toggle('active', isOndevice);
    aiProviderOllamaFields.style.display = isOndevice ? 'none' : '';
    if (aiProviderOndeviceFields) aiProviderOndeviceFields.style.display = isOndevice ? '' : 'none';
    if (aiProviderCloudFields) aiProviderCloudFields.style.display = 'none';
    if (isOndevice) refreshOndeviceStatusHint();
  }
  ...
  async function callVision(prompt, dataUrl){
    ...
    if (provider === 'ondevice'){
      ...
      throw new Error('온디바이스 모델은 아직 사진 설명(비전)을 지원하지 않아요. 설정 > AI 연결에서 내 컴퓨터(Ollama)를 선택해 비전 모델을 등록해주세요.');
    }
  ...
  /* =========================================================
     DuckDuckGo 웹 검색
     - 우선 같은 출처의 /api/search 백엔드가 있으면 그걸 사용함
       (직접 서버를 띄워서 쓰는 경우 대비)
     - GitHub Pages 같은 정적 배포에는 그런 백엔드가 없어서 항상
       실패했던 게 버그였음 → 백엔드가 없거나 실패하면 브라우저에서
       DuckDuckGo HTML 결과 페이지를 공개 CORS 프록시로 직접 가져와
       파싱하는 방식으로 자동 폴백함 (키·서버 불필요)
  ========================================================= */
  async function searchWebViaBackend(query, n){
    const res = await fetch(`/api/search?q=${encodeURIComponent(query)}&n=${n}`, { signal: AbortSignal.timeout(3500) });
  ...
  // DuckDuckGo HTML 검색 결과 페이지를 프록시로 우회해서 가져올 때 쓰는
  // 공개 CORS 프록시들 — 하나가 막히거나 느리면 다음 걸로 순서대로 재시도함
  ...
  // DuckDuckGo HTML 결과의 result__a(제목/링크), result__snippet(요약)을 파싱.
  // 제목 링크는 실제 주소가 아니라 //duckduckgo.com/l/?uddg=<인코딩된실제주소> 형태의
  // 리다이렉트라서 uddg 파라미터를 풀어서 진짜 링크를 꺼냄.
  function parseDuckDuckGoHtml(html, limit){
    ...
    nodes.forEach(node => {
      ...
      if (uddgMatch){
        try{ link = decodeURIComponent(uddgMatch[1]); }catch(e){ /* 그대로 둠 */ }
  ...
  async function searchWebViaBrowser(query, n){
    ...
    for (const buildProxyUrl of DDG_CORS_PROXIES){
      try{
        const res = await fetch(buildProxyUrl(target), { signal: AbortSignal.timeout(8000) });
        if (!res.ok) throw new Error(`프록시 HTTP ${res.status}`);
        const html = await res.text();
        const results = parseDuckDuckGoHtml(html, n);
        if (results.length) return results;
        lastErr = new Error('검색 결과를 찾지 못했어요');
      }catch(err){
        lastErr = err;
      }
    }
    throw lastErr || new Error('검색 프록시에 모두 연결하지 못했어요');
  }

  async function searchWeb(query, count){
    const n = Math.min(20, Math.max(5, count || 5));
    try{
      return await searchWebViaBackend(query, n);
    }catch(err){
      // /api/search가 없는 정적 배포(GitHub Pages 등)이거나 실패한 경우 →
      // 브라우저에서 직접 DuckDuckGo를 검색하는 방식으로 폴백
      return await searchWebViaBrowser(query, n);
    }
  }
...
- 풍속: ${c.wind_speed_10m}m/s${maxPop !== null ? `\n- 앞으로 6시간 내 최고 강수확률: ${maxPop}%` : ''}`,
  ...
  /* =========================================================
     최신 정보·사실 확인용 컨텍스트 수집(날씨 API 우선, 아니면 DuckDuckGo 검색) —
     자료 검색용 모델(callResearch)과 온디바이스 폴백(callOnDevice) 둘 다에서 재사용
  ========================================================= */
  async function gatherResearchContext(prompt, logSuffix){
    ...
    if (!weatherQuery || resultsText.startsWith('(검색 실패')){
      ...
      try{
        ...
        logExec(true, `DuckDuckGo 검색${suffix}`, searchCmd, `결과 ${results.length}건\n\n${resultsText.slice(0, 300)}`);
        addResourcesFromSearchResults(results, prompt);
      }catch(err){
        console.warn('[DuckDuckGo 검색 실패]', err);
        logExec(false, `DuckDuckGo 검색${suffix}`, searchCmd, err.message || String(err));
      }
    }

    return { resultsText, weatherQuery };
  }
  function buildResearchSystemPrompt(weatherQuery, resultsText){
    return buildSystemPrompt() + (weatherQuery && !resultsText.startsWith('(검색 실패')
  ...
  async function callConvo(prompt, hist){
    const provider = getAiProvider();
    if (provider === 'ondevice'){ setPipelineStep('convo','skip'); throw new SkipProvider(); }
    const isCloud = provider === 'cloud';
  ...
  async function callResearch(prompt, hist){
    const provider = getAiProvider();
    if (provider === 'ondevice'){ setPipelineStep('research','skip'); throw new SkipProvider(); }
    const isCloud = provider === 'cloud';
    ...
    const { resultsText, weatherQuery } = await gatherResearchContext(prompt);
    setCaption('검색 결과로 답변 정리 중…', true);
    const systemContent = buildResearchSystemPrompt(weatherQuery, resultsText);
  ...
  /* =========================================================
     ...
     - 최신 정보가 필요한 질문이면 여기서도 DuckDuckGo 검색 결과를 함께 참고해서 답함
  ========================================================= */
  const ONDEVICE_MODEL_ID = 'onnx-community/Qwen2.5-0.5B-Instruct';
  function supportsWebGPU(){
    return typeof navigator !== 'undefined' && !!navigator.gpu;
  }
  let onDevicePipeline = null;
  let onDeviceDeviceUsed = null; // 준비가 끝나면 'webgpu' | 'wasm' 로 채워짐
  function refreshOndeviceStatusHint(){
    if (!ondeviceStatusHint) return;
    if (onDeviceDeviceUsed){
      ondeviceStatusHint.textContent = onDeviceDeviceUsed === 'webgpu'
        ? '준비 완료 · WebGPU로 GPU 가속 실행 중이에요.'
        : '준비 완료 · WebGPU를 지원하지 않아 WASM(CPU)으로 실행 중이에요.';
    }else{
      ondeviceStatusHint.textContent = supportsWebGPU()
        ? '이 브라우저는 WebGPU를 지원해요. 준비하기를 누르면 미리 받아둬서 첫 대화가 더 빨라져요.'
        : '이 브라우저는 WebGPU를 지원하지 않아요 — CPU(WASM)로 실행돼요(다소 느릴 수 있어요). Chrome/Edge 최신 버전을 권장해요.';
    }
  }
  async function getOnDevicePipeline(){
    if (!onDevicePipeline){
      onDevicePipeline = (async () => {
        const { pipeline } = await import('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3/+esm');
        const progressCallback = (p) => {
          if (p?.status === 'progress' && ondeviceStatusHint){
            const pct = p.total ? Math.round((p.loaded / p.total) * 100) : null;
            ondeviceStatusHint.textContent = `모델 다운로드 중… ${p.file || ''}${pct !== null ? ` (${pct}%)` : ''}`.trim();
          }
        };
        if (supportsWebGPU()){
          try{
            const pipe = await pipeline('text-generation', ONDEVICE_MODEL_ID, { device:'webgpu', dtype:'q4f16', progress_callback: progressCallback });
            onDeviceDeviceUsed = 'webgpu';
            refreshOndeviceStatusHint();
            return pipe;
          }catch(err){
            console.warn('[온디바이스] WebGPU 초기화 실패 — WASM(CPU)으로 전환', err);
          }
        }
        const pipe = await pipeline('text-generation', ONDEVICE_MODEL_ID, { dtype:'q4', progress_callback: progressCallback });
        onDeviceDeviceUsed = 'wasm';
        refreshOndeviceStatusHint();
        return pipe;
      })();
    }
    return onDevicePipeline;
  }
  ondeviceWarmupBtn?.addEventListener('click', async () => {
    ondeviceWarmupBtn.disabled = true;
    if (ondeviceStatusHint) ondeviceStatusHint.textContent = '모델을 준비하는 중… (최초 1회만 다운로드)';
    try{
      await getOnDevicePipeline();
    }catch(err){
      console.error('[온디바이스 준비 실패]', err);
      if (ondeviceStatusHint) ondeviceStatusHint.textContent = `준비 실패: ${err.message || err}`;
    }finally{
      ondeviceWarmupBtn.disabled = false;
    }
  });
  async function callOnDevice(prompt, hist){
    ...
    if (needsResearch){
      const ctx = await gatherResearchContext(prompt, ' (온디바이스 폴백)');
      systemContent = buildResearchSystemPrompt(ctx.weatherQuery, ctx.resultsText);
      weatherQuery = ctx.weatherQuery;
      setCaption('온디바이스 모델이 검색 결과로 답변 정리 중… (최초 1회만 다운로드)', true);
    }else{
      setCaption('온디바이스 모델 준비 중… (최초 1회만 다운로드)', true);
    }
    const pipe = await getOnDevicePipeline();
    ...
    const modelOutput = await pipe(messages, { max_new_tokens: 180 });
    const generated = modelOutput?.[0]?.generated_text;
    const text = Array.isArray(generated) ? generated.at(-1)?.content : generated;
    const cmd = `온디바이스 추론 (${ONDEVICE_MODEL_ID}, ${onDeviceDeviceUsed === 'webgpu' ? 'WebGPU' : 'WASM'})`;
    if (!text){
    ...
    return { text: text.trim(), tier:'ondevice', label: needsResearch
      ? (weatherQuery ? '온디바이스 · 실시간 날씨' : '온디바이스 · 자료 검색')
      : '온디바이스' };
  }
  ...
  attachInput.addEventListener('change', async () => {
    ...
    if (isImageFile(file)){
      const provider = getAiProvider();
      const visionModel = provider === 'cloud' ? getCloudVisionModel() : (provider === 'ondevice' ? '' : getOllamaVisionModel());
      if (!visionModel){
        showAttachChip(file.name, provider === 'ondevice'
          ? '온디바이스 모델은 사진 설명을 지원하지 않아요'
          : '비전 모델을 설정에서 먼저 등록해주세요 (AI 연결 탭)');
        pendingAttachment = null;
        return;
      }
  ...
  document.getElementById('save-settings').addEventListener('click', () => {
    const selectedProvider = providerTabOndevice?.classList.contains('active') ? 'ondevice' : 'ollama';
