  ...
  const webgpuSupportHint = document.getElementById('webgpu-support-hint');
  ...
  // 클라우드 API 탭 = 브라우저에서 WebGPU로 직접 돌리는 온디바이스 모델 (키·서버 불필요)
  const ON_DEVICE_MODEL_ID = 'onnx-community/Qwen2.5-0.5B-Instruct';
  ...
  async function callVision(prompt, dataUrl){
    ...
    if (isCloud){
      ...
      throw new Error('WebGPU 온디바이스 모드에서는 사진 설명 기능을 아직 지원하지 않아요. 설정 > AI 연결에서 "내 컴퓨터(Ollama)" 탭으로 바꿔주세요.');
    }
    const model = getOllamaVisionModel();
    if (!model){
      setPipelineStep('vision','skip');
      throw new Error('비전용 모델이 설정되지 않았어요. 설정 > AI 연결에서 llava 또는 minicpm-v를 등록해주세요.');
    }
    setPipelineStep('vision','active');
    setCaption('사진을 살펴보는 중…', true);
    const cmd = `POST ${getOllamaUrl().replace(/\/$/, '')}/api/chat\nmodel: ${model} (vision)`;
    try{
      const text = await ollamaVisionDescribe(model, prompt, dataUrl);
      ...
      return { text, tier:'vision', label: '로컬 · 사진 설명' };
    }catch(err){
...
- 풍속: ${c.wind_speed_10m}m/s${maxPop !== null ? `\n- 앞으로 6시간 내 최고 강수확률: ${maxPop}%` : ''}`,
  ...
  /* =========================================================
     온디바이스 모델 공통 호출 함수 (transformers.js, 브라우저 안에서 실행)
     - device: 'webgpu'를 넘기면 WebGPU 가속으로 돌리고(클라우드 API 탭),
       비워두면 프레임워크 기본값(대개 CPU/wasm)으로 돌림(최종 폴백용).
       device별로 파이프라인을 따로 캐싱해서 재사용함.
  ========================================================= */
  const onDevicePipelines = {};
  async function getOnDevicePipeline(device){
    const key = device || 'default';
    if (!onDevicePipelines[key]){
      onDevicePipelines[key] = (async () => {
        const { pipeline } = await import('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3/+esm');
        const opts = { dtype: 'q4' };
        if (device) opts.device = device;
        return pipeline('text-generation', ON_DEVICE_MODEL_ID, opts);
      })();
    }
    return onDevicePipelines[key];
  }
  async function runOnDeviceChat(systemContent, hist, prompt, device){
    const pipe = await getOnDevicePipeline(device);
    const messages = [
      { role:'system', content: systemContent },
      ...hist,
      { role:'user', content: prompt }
    ...
    if (!text) throw new Error('온디바이스: 빈 응답');
    return text.trim();
  }
  ...
  /* =========================================================
     공급자 1: 대화용 모델
     - 내 컴퓨터(Ollama): 등록한 모델로 원격 호출
     - 클라우드 API(=WebGPU 온디바이스): 브라우저 안에서 WebGPU로 직접 생성
  ========================================================= */
  async function callConvo(prompt, hist){
    const isCloud = getAiProvider() === 'cloud';
    if (isCloud){
      setPipelineStep('convo','active');
      setCaption('브라우저에서 WebGPU로 답변 준비 중… (최초 1회만 다운로드)', true);
      const cmd = `온디바이스 추론 (WebGPU, ${ON_DEVICE_MODEL_ID})`;
      try{
        const text = await runOnDeviceChat(buildSystemPrompt(), hist, prompt, 'webgpu');
        setPipelineStep('convo','ok');
        setPipelineStep('research','skip');
        setPipelineStep('ondevice','skip');
        logExec(true, '대화 모델 호출 (WebGPU)', cmd, text.slice(0, 300));
        return { text, tier:'convo', label:'브라우저(WebGPU) · 대화' };
      }catch(err){
        setPipelineStep('convo','fail');
        logExec(false, '대화 모델 호출 (WebGPU)', cmd, err.message || String(err));
        throw err;
      }
    }
    const model = getOllamaConvoModel();
    if (!model){ setPipelineStep('convo','skip'); throw new SkipProvider(); }
    setPipelineStep('convo','active');
    setCaption('대화 모델에게 물어보는 중…', true);
    const cmd = `POST ${getOllamaUrl().replace(/\/$/, '')}/api/chat\nmodel: ${model} · think:false`;
    try{
      const text = await ollamaChat(model, buildSystemPrompt(), hist, prompt);
      setPipelineStep('convo','ok');
      setPipelineStep('research','skip');
      setPipelineStep('ondevice','skip');
      logExec(true, '대화 모델 호출', cmd, text.slice(0, 300));
      return { text, tier:'convo', label:'로컬 · 대화' };
    }catch(err){
      setPipelineStep('convo','fail');
  ...
  /* =========================================================
     공급자 2: 자료 검색용 모델 + DuckDuckGo
     - 내 컴퓨터(Ollama): 등록한 모델로 원격 호출
     - 클라우드 API(=WebGPU 온디바이스): 브라우저 안에서 WebGPU로 직접 생성
  ========================================================= */
  async function callResearch(prompt, hist){
    const isCloud = getAiProvider() === 'cloud';
    const model = isCloud ? null : getOllamaResearchModel();
    if (!isCloud && !model){ setPipelineStep('research','skip'); throw new SkipProvider(); }
    ...
    const chatCmd = isCloud
      ? `온디바이스 추론 (WebGPU, ${ON_DEVICE_MODEL_ID})`
      : `POST ${getOllamaUrl().replace(/\/$/, '')}/api/chat\nmodel: ${model} · think:false`;
    try{
      const text = isCloud
        ? await runOnDeviceChat(systemContent, hist, prompt, 'webgpu')
      ...
      return { text, tier:'research', label: weatherQuery
        ? (isCloud ? '브라우저(WebGPU) · 실시간 날씨' : '로컬 · 실시간 날씨')
        : (isCloud ? '브라우저(WebGPU) · 자료 검색' : '로컬 · 자료 검색') };
    }catch(err){
  ...
  /* =========================================================
     ...
     - Ollama나 WebGPU 시도가 실패했을 때의 마지막 안전망이라, 넓은 호환성을
       위해 기본(CPU/wasm) 장치로 돌림. 최신 정보가 필요한 질문이면 여기서도
       DuckDuckGo 검색 결과를 함께 참고해서 답함.
  ========================================================= */
  async function callOnDevice(prompt, hist){
    ...
    const cmd = `온디바이스 추론 (${ON_DEVICE_MODEL_ID}, q4)`;
    try{
      const text = await runOnDeviceChat(systemContent, hist, prompt);
      setPipelineStep('ondevice','ok');
      logExec(true, '온디바이스 모델 추론', cmd, text.slice(0, 300));
      return { text, tier:'ondevice', label: needsResearch
        ? (weatherQuery ? '온디바이스 · 실시간 날씨' : '온디바이스 · 자료 검색')
        : '온디바이스' };
    }catch(err){
      setPipelineStep('ondevice','fail');
      logExec(false, '온디바이스 모델 추론', cmd, err.message || String(err));
      throw err;
    }
  }
  ...
  function isDeepseekModelSelected(){
    // 클라우드 API(WebGPU 온디바이스) 모드는 모델을 직접 고르지 않으므로 해당 없음
    if (getAiProvider() === 'cloud') return false;
    const convo = getOllamaConvoModel().toLowerCase();
    const research = getOllamaResearchModel().toLowerCase();
  ...
  attachInput.addEventListener('change', async () => {
    ...
    if (isImageFile(file)){
      if (getAiProvider() === 'cloud'){
        showAttachChip(file.name, 'WebGPU 온디바이스 모드는 사진 설명을 지원하지 않아요 (내 컴퓨터 탭 이용)');
        pendingAttachment = null;
        return;
      }
  ...
  /* =========================================================
     설정 모달 — 브라우저 WebGPU 지원 확인 & 온디바이스 모델 미리 준비
  ========================================================= */
  async function testWebGpuSupport(){
    if (!webgpuSupportHint) return;
    if (!('gpu' in navigator)){
      webgpuSupportHint.textContent = '이 브라우저는 WebGPU를 지원하지 않아요 — CPU 모드로 더 느리게 동작해요.';
      return;
    }
    webgpuSupportHint.textContent = '확인 중… (모델을 처음 받는 경우 시간이 걸릴 수 있어요)';
    try{
      const adapter = await navigator.gpu.requestAdapter();
      if (!adapter) throw new Error('사용 가능한 GPU 어댑터를 찾지 못했어요');
      await getOnDevicePipeline('webgpu');
      webgpuSupportHint.textContent = 'WebGPU 사용 가능 · 모델 준비 완료';
    }catch(e){
      webgpuSupportHint.textContent = `WebGPU를 사용할 수 없어요(${e.message || e}) — CPU 모드로 자동 대체돼요.`;
    }
  }
  ...
  function openSettings(){
    ...
    if (webgpuSupportHint) webgpuSupportHint.textContent = '아직 확인하지 않았어요';
